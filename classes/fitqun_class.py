...
class fitqun():
    ...
    """
    def __init__(self, input_path, output_path, fitqun_directories, doAll=False, transformOnly=False) -> None:
        if doAll:
            skdetsim_options = SKDETSimOptions()
            skdetsim_options = skdetsim_options.load_options(input_path,"sk_options.pkl")
            self.zbs_files = glob.glob(input_path+"*.zbs")
            print(f"label: {skdetsim_options.particle}")
            self.labels=np.ones(len(self.zbs_files))*(convert_pdg_to_ml_label(int(skdetsim_options.particle)))
            print(self.labels)
        elif not transformOnly:
            ...
            self.labels = np.zeros(len(self.rootfiles))
